...
function generateItem(type, filePath, cmdLine, description, iconPath) {
  ...
  let cmdLineDesc = cmdLine.join(" ");
  // 显示的时候, 去掉 npm run 的前缀 ,
  // 运行时 会再用 join(" ") 拼接, 所以不影响执行
  if (cmdLine[0] == "npm") {
    cmdLineDesc = cmdLine[2];
  }
  index++;
  return {
    key: type,
