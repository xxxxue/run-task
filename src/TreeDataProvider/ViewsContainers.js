...
class ViewsContainers {
  constructor(context) {
    ...
    this.loaderList = this.setupLoaders(
      vscode.workspace.getConfiguration("runtask"),
    );
  }
  // 运行按钮
  run(item) {
    // 寻找对应的 loader , 使用 key 来匹配 ("NPM", "GULP", "SCRIPT")
    let loader = this.findLoader(this.loaderList, item);
    if (loader) {
      // 终端执行命令
      loader.runTerminal(item);
    }
  }
  setLogs(type, tree) {
    if (!tree) return;
  ...
  removeWorkSpace(workspace) {
    ...
    const index = currentWorkspace.findIndex(
      (tasks) => tasks.name === workspace.name,
    );
  ...
  setupLoaders(globalConfig) {
    ...
    const index = currentState.findIndex(
      (item) => item.name === vscode.workspace.name,
    );
    if (index > -1) {
    ...
    for (const task of loaderList) {
      task.loadTask((tasks) => {
        //将历史的操作记录数据移植到新 treeData 中
        if (preWorkspaceTreeChildren) {
          for (let item of tasks) {
            let preTask = preWorkspaceTreeChildren.find(
              (task) => task.cmdLineDesc === item.cmdLineDesc,
            );
            if (preTask) {
